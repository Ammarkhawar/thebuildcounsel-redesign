  ...
  return (
    <section id="results" ref={ref} className="section-pad bg-dark/50">
      <div className="max-w-site">
          ...
          {caseStudies.map((cs, i) => (
            <motion.div
              ...
              className={`relative rounded-2xl overflow-hidden group ${
                cs.featured
                  ? "bg-gradient-to-br from-gold/15 via-dark-2 to-dark-2 border border-gold/30"
                  : "bg-dark-2 border border-gold/10 hover:border-gold/20"
            ...
            >
              ...
              <div className="p-7">
                ...
                <div className="h-8 mb-5 flex items-center">
                  <div className="h-6 px-3 bg-dark-3 rounded flex items-center justify-center">
                    <span className="text-xs font-sans font-semibold text-muted">{cs.logoAlt}</span>
                  </div>
                </div>
        ...
        >
          <p className="text-center text-xs text-muted/60 font-sans tracking-widest uppercase mb-6">
          ...
          <div className="flex items-center justify-center gap-10 flex-wrap opacity-40">
            {["Lawline", "TSM Law", "IPT Legal", "Morrison & Assoc.", "Collins Legal"].map((name) => (
              <span key={name} className="text-sm font-sans font-medium text-muted/80 tracking-wide">
                {name}
              </span>
            ))}
          </div>
